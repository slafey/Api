...
def main():
    ...
    parser = argparse.ArgumentParser(description='Сокращение ссылки или получение статистики кликов.')
    parser.add_argument('url', help='Ссылка для сокращения или анализа кликов')
    args = parser.parse_args()

    try:
        if is_short_link(args.url):
            clicks = count_clicks(access_token, args.url)
            print(f'Количество кликов по ссылке: {clicks}')
        else:
            short_url = shorten_link(access_token, args.url)
